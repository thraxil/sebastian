...
class ServiceTests(TestCase):
    def test_get_or_create_deck_gets(self) -> None:
        d = DeckFactory()
        deck = service.get_or_create_deck(d.name, d.user)
        self.assertEqual(deck.id, d.id)

    def test_get_or_create_deck_creates(self) -> None:
        u = UserFactory()
        deck_name = "new deck"
    ...
    def test_usercard_update(self) -> None:
        uc = UserCardFactory()
    ...
    def test_usercard_update_due(self) -> None:
        now = datetime(2023, 9, 25, 12, 0, 0, tzinfo=timezone.UTC)
    ...
    def test_usercard_update_due_with_ease(self) -> None:
        now = datetime(2023, 9, 25, 12, 0, 0, tzinfo=timezone.UTC)
    ...
    def test_usercard_update_rung(self) -> None:
        uc = UserCardFactory(rung=0)
        service.usercard_update_rung(uc, timedelta(days=6))
        self.assertEqual(uc.rung, 7)

    def test_usercard_test_wrong(self) -> None:
        now = datetime(2023, 9, 25, 12, 0, 0, tzinfo=timezone.UTC)
    ...
    def test_usercard_test_correct_first_test(self) -> None:
        now = datetime(2023, 9, 25, 12, 0, 0, tzinfo=timezone.UTC)
    ...
    def test_usercard_test_correct_retest(self) -> None:
        now = datetime(2023, 9, 25, 12, 0, 0, tzinfo=timezone.UTC)
