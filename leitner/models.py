...
class UserCard(models.Model):
    ...
    def test_correct(self):
        ...
        if q.count() == 0:
            ...
            # the intervals, make sure that it gets bumped up to at least that rung
...
def percent_right (user):
    ...
    right = UserCardTest.objects.filter(usercard__user=user,correct=True).count()
...
def priority_stats (user):
    ...
    for p in range(10,0,-1):
        yield dict(priority=p, 
                   tested=UserCard.objects.filter(user=user,priority=p,rung__gte=0).count(),
                   untested=UserCard.objects.filter(user=user,priority=p,rung=-1).count(),
                   )
...
def total_due(user):
    return UserCard.objects.filter(user=user,rung__gte=0,due__lte=datetime.now()).count()

def next_hour_due(user):
    return UserCard.objects.filter(user=user,rung__gte=0,due__lte=datetime.now() + timedelta(hours=1),
                                   due__gte=datetime.now()).count()

def next_day_due(user):
    return UserCard.objects.filter(user=user,rung__gte=0,due__lte=datetime.now() + timedelta(days=1),
                                   due__gte=datetime.now() + timedelta(hours=6)).count()

def next_six_hours_due(user):
    return UserCard.objects.filter(user=user,rung__gte=0,due__lte=datetime.now() + timedelta(hours=6),
                                   due__gte=datetime.now() + timedelta(hours=1)).count()

def next_week_due(user):
    return UserCard.objects.filter(user=user,rung__gte=0,due__lte=datetime.now() + timedelta(weeks=1),
                                   due__gte=datetime.now() + timedelta(days=1)).count()

def next_month_due(user):
    return UserCard.objects.filter(user=user,rung__gte=0,due__lte=datetime.now() + timedelta(weeks=4),
                                   due__gte=datetime.now() + timedelta(weeks=1)).count()


def overdue_dates(user):
    now = datetime.now()
    return [(now - u.due).seconds for u in UserCard.objects.filter(user=user,rung__gte=0).order_by("due") if u.due < now]

def due_dates(user):
    now = datetime.now()
    return [(u.due - now).seconds for u in UserCard.objects.filter(user=user,rung__gte=0).order_by("due") if u.due > now]
...
def clump(data,num_clumps):
    ...
    clumps = [len([x for x in data if x >= clump and x < clump + clumpsize]) for clump in range(minn,maxn,clumpsize)]
    maxclump = max(clumps)
    return ["%02x" % x for x in [int(255 - (255 * float(x) / float(maxclump))) for x in clumps]]
